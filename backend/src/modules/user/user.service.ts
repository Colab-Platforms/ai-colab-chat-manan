...
class UserService {
  ...
  async listUsers(query: any, callerRole: string, callerId: number) {
    ...
    const visibleRoles =
      callerRole === "SUPERADMIN" ? ["USER", "ADMIN"] : ["USER"]; // ADMIN can only see USER-role users
  ...
  async adminUpdateUser(userId: number, data: any, callerRole: string) {
    ...
    const isSuperAdmin = user.userRoles.some(
      (ur) => ur.role.name === "SUPERADMIN" || ur.role.name === "SUPER_ADMIN",
    );
    if (isSuperAdmin) {
      throw new ApiError(
  ...
  async getUserUsage(userId: number, query: any) {
    ...
    const summary = await prisma.usageLog.aggregate({
      ...
      _sum: {
        ...
        totalTokens: true,
        billablePromptTokens: true,
        billableCompletionTokens: true,
        billableTotalTokens: true,
      },
      _count: true,
    });
    ...
    return {
      summary: {
        ...
        billablePromptTokens: summary._sum.billablePromptTokens || 0,
        billableCompletionTokens: summary._sum.billableCompletionTokens || 0,
