...
const HeroGrid = () => {
  ...
  useEffect(() => {
    ...
    const svg = container.querySelector(".grid-pat svg") as SVGElement | null;
    const overlay = container.querySelector(".grid-overlay") as HTMLElement | null;

    if (!svg) return;

    // 1. Select concentric circles (excluding the path inside <mask>)
    const circlePaths = [
      ...Array.from(svg.querySelectorAll(".circles-group > path")),
      ...Array.from(svg.querySelectorAll(".circles-group g[mask] path")),
    ] as SVGPathElement[];

    // 2. Select static radial lines
    const radialPaths = Array.from(svg.querySelectorAll(".radial-lines-group path")) as SVGPathElement[];

    const timeouts: NodeJS.Timeout[] = [];

    // --- Start Entrance Animation ---

    // Phase A: Concentric Circles (Bottom to Top)
    circlePaths.forEach((path, index) => {
      const t = setTimeout(() => {
    ...
    // Phase B: Grid Overlay Fade In (Starts at 600ms)
    if (overlay) {
      const t = setTimeout(() => {
    ...
    // Phase C: Static Radial Lines Fade In (Starts at 1200ms)
    const tRadial = setTimeout(() => {
      radialPaths.forEach((path) => path.classList.add("visible"));
    }, 1200);
    timeouts.push(tRadial);

    // Phase D: Rays Creation & Fade In (Starts at 2300ms)
    const tRays = setTimeout(() => {
      startRays();
      const raysGroup = svg.querySelector(".rays-group");
      if (raysGroup) {
        raysGroup.classList.add("visible");
      }
    }, 2300);
    timeouts.push(tRays);

    // Keep track of paths that had style properties modified, so we can clean them up.
    const modifiedOriginalPaths: SVGPathElement[] = [];

    // --- Rays Creation Function ---
    function startRays() {
      if (svg === null) return;
      // Select all original ray paths (CSS handles their color; we don't filter by stroke attr)
      const whitePaths = Array.from(
        svg.querySelectorAll('.rays-group path')
      ) as SVGPathElement[];

      whitePaths.forEach((path) => {
        path.style.opacity = "0";
        modifiedOriginalPaths.push(path);

        if (Math.random() > 0.4) {
          return;
        }

        const baseDashLength = 0.12 + Math.random() * 0.16;
        const duration = 2.0 + Math.random() * 2.0;
        const delay = Math.random() * -10;
        const baseOpacity = 0.5 + Math.random() * 0.5;
        const period = baseDashLength + 1;
        ...
        const createLayer = (
          ...
          clone.classList.add("flow-line");
          clone.setAttribute(
            "stroke-dasharray",
            `${baseDashLength * lengthMultiplier} 1`
          );
          clone.setAttribute("stroke-width", width);
          clone.setAttribute("stroke-linecap", "round");

          const delayShift = -(shift / period) * duration;
          clone.style.animationDuration = `${duration}s`;
          clone.style.animationDelay = `${delay + delayShift}s`;
          clone.style.opacity = (baseOpacity * opacityMultiplier).toString();
          clone.style.setProperty("--dash-period", period.toString());
        ...
        createLayer(1.0, "0.6", 0.12, 0.0);
        createLayer(0.75, "1.2", 0.35, baseDashLength * 0.25);
        createLayer(0.5, "1.8", 0.65, baseDashLength * 0.5);
        createLayer(0.25, "2.4", 0.95, baseDashLength * 0.75);
      });
    }
    ...
    return () => {
      ...
      // 2. Remove added classes from elements
      circlePaths.forEach((path) => path.classList.remove("visible"));
      if (overlay) {
        overlay.classList.remove("visible");
      }
      radialPaths.forEach((path) => path.classList.remove("visible"));
      const raysGroup = svg.querySelector(".rays-group");
      if (raysGroup) {
        raysGroup.classList.remove("visible");
      }

      // 3. Reset modified opacity on original white paths
      modifiedOriginalPaths.forEach((path) => {
        path.style.opacity = "";
      });

      // 4. Remove all cloned flow lines
      const flowLines = Array.from(svg.querySelectorAll(".flow-line"));
  ...
  return (
    <div ref={containerRef} className="hero-grid-wrap z-10">
        ...
        <div className="grid-pat">
          ...
          >
            <defs>
              <style type="text/css">{`
                /*
                 * CSS custom properties (defined in globals.css) drive the theme-aware
                 * stroke colors. CSS rules always beat SVG presentation attributes, so
                 * these selectors override every inline stroke="..." attribute below.
                 */
                .circles-group path,
                .circles-group g path {
                  stroke: var(--hero-grid-stroke, #DCD0FF) !important;
                  opacity: var(--hero-grid-opacity, 0.9) !important;
                  stroke-width: var(--hero-grid-stroke-width, 1.5) !important;
                }

                .radial-lines-group path {
                  stroke: var(--hero-grid-stroke, #DCD0FF) !important;
                  opacity: var(--hero-grid-opacity, 0.9) !important;
                  stroke-width: var(--hero-grid-stroke-width, 1.5) !important;
                }
                ...
                .rays-group path {
                  stroke: var(--hero-ray-stroke, #b060ff) !important;
                }
              `}</style>
            </defs>
            <g>
              <g className="circles-group">
                <path
                  strokeWidth="1"
                  className="cls-1"
                  d="M2112.82 164.258C1722.42 164.158 1323.93 189.458 949.311 303.558C735.501 367.418 444.321 531.218 764.161 708.908C1001.91 829.648 1272.25 866.798 1533.02 897.978C2017.06 944.698 2509.9 935.518 2985.97 838.268C3143.94 800.138 3416.25 738.908 3500.97 592.808C3571.16 430.158 3278.4 333.758 3164.73 296.808C2823.38 196.958 2466.52 169.388 2112.85 164.258H2112.82Z"
                  opacity="0.7"
                  style={{ transform: "none", transformOrigin: "50% 50%", transformBox: "fill-box", willChange: "transform" }}
                  stroke="#df4545ff"
                ...
                <path
                  strokeWidth="1"
                  className="cls-1"
                  d="M2281.13 201.213C2550.11 213.943 2822.39 242.863 3080.43 320.803C3212.88 359.423 3514.87 484.702 3333.31 645.352C3178.89 766.452 2955.73 804.783 2767.45 840.213C2305.38 911.183 1831.38 911.473 1370.7 840.573C1196.09 810.433 1019.08 773.923 863.341 688.843C744.731 622.023 691.431 523.263 819.931 429.073C1151.95 207.953 1882.28 185.353 2281.17 201.183L2281.13 201.213Z"
                  opacity="0.7"
                  style={{ transform: "none", transformOrigin: "50% 50%", transformBox: "fill-box", willChange: "transform" }}
                  stroke="#df4545ff"
                ...
                <path
                  strokeWidth="1"
                  className="cls-1"
                  d="M1946.21 225.263C1693.01 232.703 1437.1 256.583 1192.1 320.933C1067.33 354.983 724.14 463.713 892.52 632.593C1049.41 761.753 1318.36 801.433 1515.17 831.763C1777.27 866.013 2042.26 873.613 2304.78 860.353C2531.64 847.583 2758.65 821.893 2976.44 759.713C3534.79 588.963 3250.1 380.283 2852.2 298.893C2554.33 235.033 2249.55 218.523 1946.21 225.293V225.263Z"
                  opacity="0.7"
                  style={{ transform: "none", transformOrigin: "50% 50%", transformBox: "fill-box", willChange: "transform" }}
                  stroke="#df4545ff"
                ></path>
                <path
                  strokeWidth="1"
                  className="cls-1"
                  d="M1959.75 261.583C1734.84 268.193 1507.5 289.413 1289.87 346.563C1179.03 376.833 874.181 473.423 1023.75 623.383C1164.6 738.693 1400.84 772.972 1576.83 800.312C1809.68 830.743 2045.06 837.493 2278.24 825.713C2479.79 814.363 2681.44 791.533 2874.86 736.293C3370.89 584.593 3117.89 399.203 2764.48 326.953C2499.85 270.233 2229.19 255.563 1959.68 261.583H1959.75Z"
                  opacity="0.7"
                  style={{ transform: "none", transformOrigin: "50% 50%", transformBox: "fill-box", willChange: "transform" }}
                  stroke="#df4545ff"
                ></path>
                <path
                  strokeWidth="1"
                  className="cls-1"
                  d="M1972.5 296.018C1774.24 301.838 1573.77 320.558 1381.97 370.928C1284.2 397.608 1015.55 482.757 1147.41 614.977C1273.4 717.357 1478.28 746.377 1634.97 770.917C1847.2 798.587 2061.89 804.207 2274.28 792.107C2449.59 781.187 2624.99 760.457 2792.61 710.417C3222.14 573.157 2974.77 412.938 2681.94 353.658C2448.6 303.648 2210.05 290.718 1972.5 296.018Z"
                  opacity="0.7"
                  style={{ transform: "none", transformOrigin: "50% 50%", transformBox: "fill-box", willChange: "transform" }}
                  stroke="#df4545ff"
                ></path>
                <path
                  strokeWidth="1"
                  className="cls-1"
                  d="M2093.91 325.603C1866.3 325.533 1633.78 340.312 1415.5 406.832C1290.82 443.652 1121.32 540.442 1307.56 643.182C1446.23 713.582 1603.91 735.233 1755.86 753.393C2038.21 780.663 2325.5 775.273 2603.02 718.583C2698.18 694.963 2846.96 663.413 2902 577.843C2948.05 480.103 2769.06 422.693 2701.4 401.103C2503.93 344.383 2298.2 328.493 2093.94 325.562L2093.91 325.592V325.603Z"
                  opacity="0.7"
                  style={{ transform: "none", transformOrigin: "50% 50%", transformBox: "fill-box", willChange: "transform" }}
                  stroke="#df4545ff"
                ></path>
                <path
                  strokeWidth="1"
                  className="cls-1"
                  d="M2283.57 359.59C2423.79 371.73 2566.78 390.61 2698.18 442.17C2817.61 489.81 2871.96 568.11 2731.05 638.91C2431.17 774.65 1869.89 767.64 1551.37 690.59C1426.83 662.59 1190.56 565.97 1407.33 455.42C1655.03 346.52 2015.28 341.36 2283.57 359.58V359.59Z"
                  opacity="0.7"
                  style={{ transform: "none", transformOrigin: "50% 50%", transformBox: "fill-box", willChange: "transform" }}
                  stroke="#df4545ff"
                ></path>
                <path
                  strokeWidth="1"
                  className="cls-1"
                  d="M2088.54 371.827C1905.27 371.827 1717.81 383.637 1542.31 438.287C1448.67 466.807 1317.21 541.197 1455.68 621.837C1558.75 676.847 1677.3 694.707 1791.03 709.777C2019.83 735.137 2252.87 731.787 2478.34 688.557C2560.29 669.307 2675.09 646.907 2729.07 579.297C2781.71 491.327 2605.25 441.247 2546.06 424.437C2396.45 385.777 2242.23 373.767 2088.54 371.797V371.827Z"
                  opacity="0.7"
                  style={{ transform: "none", transformOrigin: "50% 50%", transformBox: "fill-box", willChange: "transform" }}
                  stroke="#df4545ff"
                ></path>
                <path
                  strokeWidth="1"
                  className="cls-1"
                  d="M2006.62 388.371C1879.64 392.091 1751.11 404.101 1628.35 436.371C1565.2 453.841 1394.12 507.302 1478.11 592.712C1562.9 659.922 1686.78 675.812 1790.44 692.592C2035.54 721.282 2289.04 719.701 2527.65 655.181C2805.16 568.921 2654.3 464.201 2461.08 425.311C2311.54 393.271 2158.83 385.011 2006.65 388.401L2006.62 388.371Z"
                  opacity="0.7"
                  style={{ transform: "none", transformOrigin: "50% 50%", transformBox: "fill-box", willChange: "transform" }}
                  stroke="#df4545ff"
                ></path>
                <path
                  strokeWidth="1"
                  className="cls-1"
                  d="M2011.69 401.438C1895.29 404.858 1777.46 415.878 1664.94 445.428C1607.04 461.448 1450.21 510.438 1527.22 588.738C1605.62 650.618 1717.94 664.738 1813.53 680.298C2038.21 706.588 2270.59 705.138 2489.31 645.988C2743.73 566.898 2605.35 470.898 2428.27 435.268C2291.15 405.888 2151.22 398.318 2011.69 401.418V401.448V401.438Z"
                  opacity="0.7"
                  style={{ transform: "none", transformOrigin: "50% 50%", transformBox: "fill-box", willChange: "transform" }}
                  stroke="#df4545ff"
                ></path>
                <path
                  strokeWidth="1"
                  className="cls-1"
                  d="M2016.5 415.518C1908.89 418.678 1799.93 428.908 1696.01 456.708C1642.25 471.608 1502.88 517.148 1575.22 587.978C1744.56 714.548 2265.52 702.308 2464.34 635.388C2688.99 560.538 2543.33 475.198 2397.11 446.448C2271.81 419.598 2143.98 412.688 2016.47 415.518H2016.5Z"
                  opacity="0.7"
                  style={{ transform: "none", transformOrigin: "50% 50%", transformBox: "fill-box", willChange: "transform" }}
                  stroke="#df4545ff"
                ></path>
                <path
                  strokeWidth="1"
                  className="cls-1"
                  d="M2021.05 433.511C1924.17 436.341 1826.05 445.521 1732.44 470.131C1684.21 483.491 1553.74 524.281 1617.84 589.391C1768.41 705.431 2238.87 693.851 2418.59 637.061C2630.12 571.621 2515 491.021 2367.8 461.671C2253.63 437.231 2137.22 430.911 2021.08 433.511H2021.05Z"
                  opacity="0.7"
                  style={{ transform: "none", transformOrigin: "50% 50%", transformBox: "fill-box", willChange: "transform" }}
                  stroke="#df4545ff"
                ></path>
                <path
                  strokeWidth="1"
                  className="cls-1"
                  d="M2491.4 570.645C2480.96 627.185 2346.39 652.555 2294.39 661.485C2157.04 682.335 2015.76 684.885 1877.77 667.975C1585.54 627.505 1550.67 515.525 1867.2 468.815C2022.57 448.805 2182.04 449.895 2335.9 480.915C2379.69 489.875 2491.18 517.065 2491.4 570.645Z"
                  opacity="0.7"
                  style={{ transform: "none", transformOrigin: "50% 50%", transformBox: "fill-box", willChange: "transform" }}
                  stroke="#df4545ff"
                ></path>
                <g mask="url(#mask0_1541_20745)">
                  <path
                    strokeWidth="1"
                    className="cls-1"
                    d="M2079.48 484.484C1972.26 485.204 1785.5 491.784 1699.66 557.294C1624.13 635.294 1864.52 673.064 1909.98 677.044C2036.82 691.194 2165.81 689.514 2290.85 667.014C2362 656.384 2539.61 600.194 2396.32 533.574C2295.99 493.504 2186.24 486.894 2079.48 484.484Z"
                    opacity="0.7"
                    style={{ transform: "none", transformOrigin: "50% 50%", transformBox: "fill-box", willChange: "transform" }}
                    stroke="#df4545ff"
                  ></path>
                  <path
                    strokeWidth="1"
                    className="cls-1"
                    d="M2033.4 520.959C1961.46 523.069 1888.53 529.909 1819.1 548.499C1783.59 558.539 1691.29 587.979 1737.41 635.389C1851.18 721.229 2193.19 712.139 2327.68 669.609C2481.3 621.279 2392.47 562.059 2287.92 541.599C2204.06 523.639 2118.68 519.029 2033.4 520.939V520.969V520.959Z"
                    opacity="0.7"
                    style={{ transform: "none", transformOrigin: "50% 50%", transformBox: "fill-box", willChange: "transform" }}
                    stroke="#df4545ff"
                  ></path>
                  <path
                    strokeWidth="1"
                    className="cls-1"
                    d="M2037.32 565.545C1973.68 567.425 1909.15 573.475 1847.75 589.895C1816.73 598.845 1735.89 623.975 1774.66 665.955C1873.64 742.705 2174.91 734.255 2293.35 698.325C2431.67 656.775 2360.71 603.175 2262.42 583.805C2188.25 567.915 2112.72 563.835 2037.29 565.515L2037.32 565.545Z"
                    opacity="0.7"
                    style={{ transform: "none", transformOrigin: "50% 50%", transformBox: "fill-box", willChange: "transform" }}
                    stroke="#df4545ff"
                  ></path>
                  <path
                    strokeWidth="1"
                    className="cls-1"
                    d="M2075.69 618.516C1999.6 619.406 1878.65 622.856 1816.17 667.636C1761.36 722.346 1930.63 748.176 1962.45 750.906C2047.6 760.116 2134.13 758.936 2218.06 744.196C2265.55 737.486 2391.55 698.996 2294.38 652.836C2225.27 624.676 2149.41 620.296 2075.73 618.556L2075.69 618.526V618.516Z"
                    opacity="0.7"
                    style={{ transform: "none", transformOrigin: "50% 50%", transformBox: "fill-box", willChange: "transform" }}
                    stroke="#df4545ff"
                  ></path>
              ...
              <g className="rays-group">
                <path
                  ...
                  opacity="0.9954864224710036"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.6569061378540937px 1px"
                  stroke="#b060ff"
                ...
                <path
                  ...
                  opacity="0.2417532550462056"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.1248808104719501px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.5150126425724011"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.842925221513724px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.17964591426425613"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.09228515625px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.03544352625613101"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.9905027063505258px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.7808548940520268"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.4418894489936065px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.8879566783143673"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.5237957182980608px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.9587773740349803"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.5950636007764842px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.9999784022511449"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.683250382571714px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.9495525505917612"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.7009830760362092px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.9020664545532782"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.5362266433949117px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.25655074170208536"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.925042070884956px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.9970868341915775"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.6631880720087793px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.5274535421340261"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.8390551149204839px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.42082961861160584"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.8730956740037072px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.22535447162226774"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.11642446022597142px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.278223591420101"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.14407408205443062px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.34029879243462346"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.17791548868990503px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.041853810398606583"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.9886835441284347px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.3201881706772838"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.9047949431987945px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.006717080337693915"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.9982756840472575px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.41070632488117553"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.21667710368637927px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.10023140549310483"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.050792190741049126px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
                <path
                  ...
                  opacity="0.31195067302905954"
                  pathLength="1"
                  strokeDashoffset="0px"
                  strokeDasharray="0.9074915722303558px 1px"
                  stroke="#b060ff"
                  style={{ willChange: "transform" }}
                ></path>
              </g>
              <g className="radial-lines-group">
                <path
                  className="cls-1"
                  strokeWidth="1"
                  d="M2401.65 6.94531C2316.2 145.945 2227.7 283.165 2146.6 424.835C2142.46 435.405 2138.97 446.175 2135.98 457.085C2126.24 492.515 2121.65 529.435 2116.46 566.165C2110.92 603.365 2106.94 640.765 2104.13 678.305H2104.12"
